...
import {
  AppSetupCommand,
  CopyEnvExampleAction,
  GenerateAppKeyAction,
  GenerateJwtSecretAction,
  IAction,
  ISetupCommand,
  QuestionData,
  QuestionIDs,
  SetupDockerDatabaseScripts,
  SetupService,
  buildQuestionDTOs,
...
ENABLE_AUTH_ROUTES=`,
  ...
  describe("crypto service", () => {
    test("services should be defined", () => {
      ...
      const packageJsonService =
        SetupService.getInstance().getPackageJsonService();
  ...
  describe("copyEnvExample action", () => {
    test("should copy the .env.example file to .env with no answer required", async () => {
      ...
      const questionDTO = questionsArray.find(
        (questionDTO) => questionDTO.id === QuestionIDs.copyEnvExample,
      );
      ...
      expect(fs.readFileSync(getOutputPath(".env"), "utf8")).toContain(
        "TEST_ENV=test",
      );
    });
  });
  ...
  describe("appKey action", () => {
    test("should generate a new app key using default value", async () => {
      ...
      const questionDTO = questionsArray.find(
        (questionDTO) => questionDTO.id === QuestionIDs.appKey,
      );
      const actionConstructor = questionDTO?.actionCtor as new () => IAction;
  ...
  describe("jwtSecret action", () => {
    test("should generate a new jwt secret using default value", async () => {
      ...
      const questionDTO = questionsArray.find(
        (questionDTO) => questionDTO.id === QuestionIDs.jwtSecret,
      );
      const actionConstructor = questionDTO?.actionCtor as new () => IAction;
    ...
    test("should not generate a new jwt secret when answer is no", async () => {
      ...
      expect(fs.existsSync(getOutputPath(".env"))).toBe(true);
      expect(fs.readFileSync(getOutputPath(".env"), "utf8")).toBe(
        "JWT_SECRET=",
      );
    });
  ...
  describe("selectDb action", () => {
    test("should select mongodb as a database", async () => {
      const questionDTO = questionsArray.find(
        (questionDTO) => questionDTO.id === QuestionIDs.selectDb,
      );
      const actionConstructors =
        questionDTO?.actionCtors as unknown as new () => IAction[];

      userQuestion = QuestionData.createWithDefinedAnswer(
        questionDTO!,
        "mongodb",
      );
      ...
      const packageJson = SetupService.getInstance()
        .getPackageJsonService()
        .getJsonSync();
      ...
      expect(envContents).toContain(
        "DATABASE_MONGODB_URI=mongodb://root:example@localhost:27018/app?authSource=admin",
      );
    });
    ...
    test("should select postgres as a database", async () => {
      ...
      expect(envContents).toContain(
        "DATABASE_POSTGRES_URI=postgres://root:example@localhost:5433/app",
      );
    });
  ...
  describe("enableExpress action", () => {
    test("should enable express using default value", async () => {
      const questionDTO = questionsArray.find(
        (questionDTO) => questionDTO.id === QuestionIDs.enableExpress,
      );
      const actionConstructor = questionDTO?.actionCtor as new () => IAction;
  ...
  describe("appPort action", () => {
    test("should set the app port using default value", async () => {
      const questionDTO = questionsArray.find(
        (questionDTO) => questionDTO.id === QuestionIDs.appPort,
      );
      const actionConstructor = questionDTO?.actionCtor as new () => IAction;
  ...
  describe("enableAuthRoutes action", () => {
    test("should enable auth routes using default value", async () => {
      const questionDTO = questionsArray.find(
        (questionDTO) => questionDTO.id === QuestionIDs.enableAuthRoutes,
      );
      const actionConstructor = questionDTO?.actionCtor as new () => IAction;
