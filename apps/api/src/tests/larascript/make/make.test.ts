...
describe(`make test suite`, () => {
  MakeServices.init({
    ...
    pathToTemplates: path.join(
      process.cwd(),
      "../../libs/make/src/make/templates",
    ),
  ...
  describe(`make types (total ${makeTypes.length})`, () => {
    ...
    for (const makeType of makeTypes) {
      test(`make ${makeType} (${currentCount}/${makeTypes.length})`, async () => {
        ...
        cmd.setParsedArguments(
          makeTestHelper.getParsedArguments(fileName, collection),
        );
        ...
        const targetFileFullPath = cmd
          .getMakeFileService()
          .getTargetDirFullPath();
