...
export class Repository<Model extends IModel> implements IRepository<Model> {
   ...
   * @returns
   */
  protected async withRelationships(
    results: IModel | ICollection,
    args: Parameters<IModel["loadRelationships"]>[0],
  ): Promise<ICollection<IModel>> {
    const resultsAsCollection =
      results instanceof Collection ? results : collect([results]);
