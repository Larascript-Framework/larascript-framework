...
describe("eloquent", () => {
  ...
  describe("test find, findOrFail, all and get as model", () => {
    ...
    test('test find, findOrFail, all and get as model (postgres)', async () => {
      await execTest(postgres);
    });

    test('test find, findOrFail, all and get as model (mongodb)', async () => {
      await execTest(mongodb);
    });
  });

  describe("test find, findOrFail, all and get", () => {
    const execTest = async (eloquentContext: EloquentContext) => {
      const query = eloquentContext.queryBuilder(TestPeopleModel);
      const inserted = await query.clone().orderBy("name", "asc").get();
    ...
    test('test find, findOrFail, all and get (postgres)', async () => {
      await execTest(postgres);
    });

    test('test find, findOrFail, all and get (mongodb)', async () => {
      await execTest(mongodb);
    });
  });

  describe("test selecting fields", () => {
    const execTest = async (eloquentContext: EloquentContext) => {
      const query = eloquentContext.queryBuilder(TestPeopleModel);
      ...
      const resultsOnlyNameAttributes =
        resultsOnlyName[0]?.getAttributes() ?? {};
      expect(Object.keys(resultsOnlyNameAttributes)).toHaveLength(2); // Name and ID
    }

    test('test selecting fields (postgres)', async () => {
      await execTest(postgres);
    });

    test('test selecting fields (mongodb)', async () => {
      await execTest(mongodb);
    });
  });

  test("test with raw sql (postgres)", async () => {
    postgres.execTest(async (eloquentContext: EloquentContext) => {
  ...
  describe("test with raw select columns", () => {
    const execTest = async (eloquentContext: EloquentContext) => {
      async (eloquentContext: EloquentContext) => {
        const query = eloquentContext.queryBuilder(TestPeopleModel);
  
        const results = await query
          .clone()
          .selectRaw('"name", "age", "createdAt"')
          .get();
  ...
  describe("test fetching model using array of ids", () => {
    const execTest = async (eloquentContext: EloquentContext) => {
      await eloquentContext.queryBuilder(TestPeopleModel)
        .insert([
          {
            name: "George",
            age: 25,
      ...
      const george = await eloquentContext.queryBuilder(TestPeopleModel)
      .where("name", "=", "George")
      .first();

      expect(george).toBeDefined();
      const roles = george?.getAttributes()?.roles;
    ...
    test('test fetching model using array of ids (postgres)', async () => {
      await execTest(postgres);
    });

    test('test fetching model using array of ids (mongodb)', async () => {
      await execTest(mongodb);
    });
  });
});
