...
export default class RouteListCommand extends BaseCommand {
  ...
  description =
    "List all available routes. Use --details to see more information.";
   ...
   */
  async execute() {
    const showDetails = this.parsedArgumenets.find((arg) =>
      ["--details", "--d", "--detailed"].includes(arg.value),
    );
    ...
    httpService.getRegisteredRoutes().forEach((route) => {
      if (showDetails) {
        ...
        this.input.writeLine(
          `  Security: [${route.security?.map((s) => s.getId()).join(", ")}]`,
        );
        this.input.writeLine();
        return;
      }
