...
export class WorkerCommand extends BaseCommand {
  ...
  async execute() {
    ...
    const intervalId = setInterval(async () => {
      await WorkerServiceProvider.worker().runWorker(options);
      WorkerServiceProvider.logger()?.console(
        "Running worker again in " + options.runAfterSeconds + " seconds",
      );
    }, 0);
  ...
  private getWorkerOptions(): TEventWorkerOptions {
    ...
    const options = WorkerServiceProvider.events().getDriverOptionsByName(
  ...
  private validateOptions(
    driverName: string,
    options: IEventDriversConfigOption["options"] | undefined,
  ) {
    if (!options) {
