import { BaseSyncSubscriber } from "../events/base/BaseSyncSubscriber.js";
...
class TestEvent extends BaseSyncSubscriber {
  ...
  describe("validatePayload", () => {
    ...
    describe("sync driver", () => {
      test("should use sync driver", () => {
        ...
        expect(event.getDriverName()).toBe(EVENT_DRIVERS.SYNC);
      });
    });

    describe("subscriber", () => {
      test("should be a subscriber", () => {
        const event = new TestEvent();
        expect(event.type).toBe("subscriber");
      });
    });
  });
});
