...
class MigrateFreshCommand extends BaseMigrationCommand {
  ...
  async execute() {
    ...
    const seed: boolean =
      typeof this.getArguementByKey("seed")?.value === "string";
  ...
  private async confirm(): Promise<boolean> {
    ...
    const answer = await this.input.askQuestion(
      "Are you sure you want to drop all tables and run fresh migrations? (y/n)\n",
    );
    return answer === "y";
  }
}
